...
class HtmlParser @Inject constructor() {
    ...
    /**
     * SSR _ROUTER_DATA 是否仍内嵌可播放 item（2026 后多数为空壳）。
     */
    fun hasPlayableItem(routerData: String): Boolean = findItemList(routerData) != null

    /**
     * 解析 /web/api/v2/aweme/iteminfo/ 返回的 JSON。
     */
    fun parseItemInfo(itemInfoJson: String, contentId: String): ContentInfo {
        val root = try {
            JSONObject(itemInfoJson)
        } catch (_: Exception) {
            throw ParseException.ParseFailed("iteminfo 响应不是合法 JSON")
        }
        val status = root.optInt("status_code", -1)
        if (status != 0) {
            val msg = root.optString("status_msg", "iteminfo 失败")
            throw ParseException.ParseFailed("iteminfo 错误 [$status]：$msg")
        }
        val item = root.optJSONArray("item_list")?.optJSONObject(0)
            ?: throw ParseException.VideoUrlNotFound("iteminfo 未返回作品数据")

        val images = extractImagesFromJson(item)
        if (images.isNotEmpty()) {
            val note = parseNoteFromItem(item, contentId, images)
        ...
        return parseVideoFromItem(item, contentId)
    }
    ...
    fun parseVideoInfo(routerData: String, videoId: String): ContentInfo.Video {
        ...
        if (videoItem != null) {
            return parseVideoFromItem(videoItem, videoId)
        }
    ...
    private fun parseVideoFromItem(videoItem: JSONObject, videoId: String): ContentInfo.Video {
    ...
    private fun extractQualities(videoObj: JSONObject?): List<VideoQuality> {
        ...
        // iteminfo 常常只有一条 720p playwm；用 play_addr.uri 补 1080/720/540
        val expanded = expandQualitiesFromPlayUri(videoObj, result)
        return expanded.sortedByDescending { it.bitRate }
    }

    private fun expandQualitiesFromPlayUri(
        videoObj: JSONObject,
        existing: List<VideoQuality>,
    ): List<VideoQuality> {
        val uri = videoObj.optJSONObject("play_addr")?.optString("uri").orEmpty()
        if (uri.isEmpty() || !uri.startsWith("v")) return existing

        val presets = listOf(
            Triple("1080p", 3_000_000, "1080p"),
            Triple("720p", 1_500_000, "720p"),
            Triple("540p", 800_000, "540p"),
        ).map { (label, bitRate, ratio) ->
            VideoQuality(
                label = label,
                bitRate = bitRate,
                url = "https://aweme.snssdk.com/aweme/v1/play/?video_id=$uri&ratio=$ratio&line=0",
            )
        }

        if (existing.size >= 2) {
            val has1080 = existing.any { q ->
                q.label.contains("1080", ignoreCase = true) ||
                    q.label.contains("超清") ||
                    q.url.contains("ratio=1080")
            }
            return if (has1080) existing else listOf(presets.first()) + existing
        }
        return presets
    }
